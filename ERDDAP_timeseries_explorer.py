...
# Inspired by [Jason Grout's excellent ESIP Tech Dive talk on "Jupyter Widgets"]
# (https://youtu.be/CVcrTRQkTxo?t=2596), this notebook uses the `ipyleaflet` and `bqplot` widgets
# to interactively explore the last two weeks of time series data from an ERDDAP Server. Select a
# `standard_name` from the list, then click a station to see the time series.
...
# `ipyleaflet` and `bqplot` are both Jupyter widgets, so can interact with Python like any other widget.
# Since we want to click on a map in a notebook and get an interactive time series plot, they are perfect
# tools to use here.
...
# To make working with ERDDAP simpler, we use `erddapy`, a high-level python interface
# to ERDDAP's RESTful API
...
# This code should work with minor modifications on any ERDDAP (v1.64+) endpoint that has
# `cdm_data_type=timeseries` or `cdm_data_type=point` datasets.  Change the values for other
# ERDDAP endpoints or regions of interest
...
# Find all the `standard_name` attributes that exist on this ERDDAP endpoint,
# using [ERDDAP's "categorize" service](http://www.neracoos.org/erddap/categorize/index.html)
...
# This function finds all the datasets with a given standard_name in the specified time period,
# and return GeoJSON
...
def alllonlat(e, cdm_data_type, min_time, max_time):
    url = "{}/tabledap/allDatasets.csv?datasetID%2CminLongitude%2CminLatitude&cdm_data_type="
    "%22{}%22&minTime%3C={}&maxTime%3E={}".format(
        e.server,
        cdm_data_type,
